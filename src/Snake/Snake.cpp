...
{
	this->direction= Direction::RIGHT; //as default value can be changed 
	this->pendingGrow =false; //as default value too
	this->body.push_front({startX, startY});
    this->bodySet.insert({startX, startY});
}
...
pair<int, int> Snake::nextHeadPosition() const
{   //build it first as we need it at the move method
	//we fisrt look at the front as we need to add heads
	pair<int, int> temp = this->body.front();
	int x = temp.first;
	int y = temp.second;
